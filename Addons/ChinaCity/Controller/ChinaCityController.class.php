...
class ChinaCityController extends AddonsController {
	...
	public function getProvince() {
		if (IS_AJAX) {
			//$pid = I('request.pid', 0, 'intval'); //默认的省份id

			//if (!empty($pid)) $map['id'] = $pid;
			$map['level'] = 1;
			$map['upid'] = 0;
	...
	public function getCity() {
		if (IS_AJAX) {
			//$cid = I('request.cid', 0, 'intval'); //默认的城市id
			$pid = I('request.pid', 0, 'intval'); //传过来的省份id
	...
	public function getDistrict() {
		if (IS_AJAX) {
			//$did = I('request.did', 0, 'intval'); //默认的城市id
			$cid = I('request.cid', 0, 'intval'); //传过来的城市id

			//if (!empty($did)) $map['id'] = $did;
			$map['level'] = 3;
			$map['upid'] = $cid;
