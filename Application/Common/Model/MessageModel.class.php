...
class MessageModel extends Model {
	...
	public function getHaventToastMessage($uid) {
		$messages = D('message')->where(array('to_uid' => $uid, 'is_read' => 0, 'last_toast' => 0))->order('id desc')->limit(99999)->select();
	...
	public function setAllToasted($uid) {
		$now = time();
		D('message')->where(array('to_uid' => $uid, 'is_read' => 0, 'last_toast' => 0))->setField('last_toast', $now);
	}

	public function setAllReaded($uid) {
		D('message')->where(array('to_uid' => $uid, 'is_read' => 0))->setField('is_read', 1);
	}
	...
	public function getHaventReadMeassage($uid, $is_toast = 0) {
		$messages = D('message')->where(array('to_uid' => $uid, 'is_read' => 0))->order('id desc')->limit(99999)->select();
		foreach ($messages as &$v) {
			$v['ctime'] = friendlyDate($v['create_time']);
			$v['content'] = op_t($v['content']);
		}
