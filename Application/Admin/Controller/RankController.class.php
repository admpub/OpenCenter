...
class RankController extends AdminController {
	...
	public function index($page = 1, $r = 20) {
		...
		foreach ($list as &$val) {
			$val['u_name'] = D('member')->where(array('uid' => $val['uid']))->getField('nickname');
	...
	public function changeTypes($id = null) {
		...
		$types = D('rank')->where(array('id' => $id))->getField('types');
		$types = $types ? 0 : 1;
		$result = D('rank')->where(array('id' => $id))->setField('types', $types);
		if ($result) {
			$this->success('设置成功');
		} else {
			$this->error('设置失败');
	...
	public function deleteRank($id = null) {
		...
		$result = D('rank')->where(array('id' => $id))->delete();
		$result1 = D('rank_user')->where(array('rank_id' => $id))->delete();
		if ($result) {
			$this->success('删除成功');
		} else {
			$this->error('删除失败');
	...
	public function editRank($id = null, $title = null, $logo = null, $types = 1) {
		...
		if (IS_POST) {
			...
			$data['types'] = $types;
			if ($isEdit) {
				$data_old = $model->where(array('id' => $id))->find();
				if ($title != '') {
	...
	public function userRankList($id = null, $page = 1) {
		...
		$u_name = D('member')->where(array('uid' => $id))->getField('nickname');
		$model = D('rank_user');
		...
		foreach ($rankList as &$val) {
			$val['title'] = D('rank')->where(array('id' => $val['rank_id']))->getField('title');
		...
		if (IS_POST) {
			...
			if ($is_Edit) {
				...
				if (!$result) {
					...
					$rank = D('rank')->where(array('id' => $data['rank_id']))->find();
	...
	public function userChangeRank($id = null, $uid = '', $reason = '', $is_show = '', $rank_id = '') {
		if (IS_POST) {
			...
			$data = D('rank_user')->where(array('id' => $id))->find();
			if (!$data) {
				$this->error('该头衔关联不存在');
			}
			$ranks = D('rank')->select();
	...
	public function deleteUserRank($id = null) {
		...
		$result = D('rank_user')->where(array('id' => $id))->delete();
		if ($result) {
			$this->success('删除成功');
		} else {
			$this->error('删除失败');
	...
	public function rankVerify($page = 1) {
		...
		foreach ($rankList as &$val) {
			...
			$val['is_show'] = $val['is_show'] ? '显示' : '不显示';
			//获取用户信息
			$u_user = D('member')->where(array('uid' => $val['uid']))->getField('nickname');
			$val['u_name'] = $u_user;
		}
		unset($val);
		$builder = new AdminListBuilder();
